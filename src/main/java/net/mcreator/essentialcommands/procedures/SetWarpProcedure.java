		...
		if (world instanceof ServerLevel _level)
			_level.getServer().getCommands().performPrefixedCommand(new CommandSourceStack(CommandSource.NULL, new Vec3(x, y, z), Vec2.ZERO, _level, 4, "", Component.literal(""), _level.getServer(), null).withSuppressedOutput(),
					("/tellraw @p {\"text\":\"Set Warp to " + "" + new java.text.DecimalFormat("##.##").format(EssentialCommandsModVariables.MapVariables.get(world).Warp1X) + " "
							+ new java.text.DecimalFormat("##.##").format(EssentialCommandsModVariables.MapVariables.get(world).Warp1Y) + " " + new java.text.DecimalFormat("##.##").format(EssentialCommandsModVariables.MapVariables.get(world).Warp1Z)
							+ "!\",\"color\":\"yellow\"}"));
	}
}
